...
| **Azure Container Instances** | ARM path (`Microsoft.ContainerInstance`) | ✅ Container group CRUD, start/stop/restart, bounded logs, instanceView, restart policies, `emptyDir`/`secret`/`azureFile` volumes, init containers; real Docker pods (shared network namespace) or mocked |
