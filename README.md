...
| **Azure Container Instances** | ARM path (`Microsoft.ContainerInstance`) | Container group CreateOrUpdate, Get (`?$expand=instanceView`), UpdateTags, Delete, List; `start`/`stop`/`restart`; bounded container logs (`tail`, `timestamps`); `Always`/`OnFailure`/`Never` restart policies owned by the emulator; `emptyDir`, `secret` and `azureFile` volumes; init containers; private-registry pulls. Each group is a Docker pod — one infrastructure container owns the network namespace so containers reach each other on `localhost` — or mocked |
