...
| **Azure Container Instances** | ARM path (`Microsoft.ContainerInstance`) | Container group CreateOrUpdate, Get, UpdateTags, Delete, List (`instanceView` on every single-resource response); `start`/`stop`/`restart`; bounded container logs (`tail`, `timestamps`); `Always`/`OnFailure`/`Never` restart policies owned by the emulator; `emptyDir`, `secret` and `azureFile` volumes; init containers; private-registry pulls. Each group is a Docker pod — one infrastructure container owns the network namespace so containers reach each other on `localhost` — or mocked |
...
| Azure Container Instances | `alpine:3.20` (infrastructure container) + each group's own images | One Docker pod per container group; the infrastructure container owns the shared network namespace and every host port binding |
